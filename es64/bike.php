...
class Bike{
    ...
    public function rental_fee(){
        $oraFine = time();
        $diff = $oraFine - $this->oraInizio;
        $diff = $diff/3600;
        $diff = ceil($diff);
        if($this->tipo == "passeggio"){
            return $diff * self::$prezzoPasseggio;
        }else if($this->tipo == "mountain-bike"){
            return $diff * self::$prezzoMountainBike;
        }else if($this->tipo == "corsa"){
            return $diff * self::$prezzoCorsa;
        }
    }
